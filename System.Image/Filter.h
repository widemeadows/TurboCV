...
namespace System
{
    namespace Image
    {
        inline double ComputeGauss(double x, double sigma)
        {
            return exp(-pow(x, 2.0) / (2 * pow(sigma, 2.0))) / (sigma * sqrt(2 * CV_PI));
        }

        inline double ComputeGaussDeriv(double x, double sigma)
        {
            return -x * ComputeGauss(x, sigma) / pow(sigma, 2);
        }
